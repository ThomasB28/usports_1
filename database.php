...
class MatchBasket{
	...
	public $assists;
		...
		$sth = $dbh->prepare ( "INSERT INTO `Stats Basket` (`match`, `joueur`, `rebonds`, `faute`, `points`, `assists`) VALUES(?,?,?,?,?,?)" );
		$sth->execute ( array (
			 $match,
                         $joueur,
                         $rebonds,
        ...
        public static function getStats($dbh,$match,$joueur){
            $query = "SELECT * FROM `Stats Basket` WHERE `Match`=? AND `Joueur`=?";
            $sth = $dbh->prepare ($query);
            $sth->execute (array($match,$joueur) );
            $users = $sth->fetchAll();
          return $users;
        }
}
